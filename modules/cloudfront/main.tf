...
resource "aws_cloudfront_distribution" "cloudfront" {
  origin {
    ...
    origin_id   = "S3-website-static-bucket"
  ...
  comment             = "CloudFront for victor_ojetokun AWS"
...
# Create a CloudFront origin access id
resource "aws_cloudfront_origin_access_identity" "origin_access_identity" {
  comment = "Origin Access Identity for naima"
}

# Attaching bucket policy to allow CloudFront authentication
